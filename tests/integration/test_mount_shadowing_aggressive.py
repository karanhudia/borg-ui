                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                for i, op in enumerate(mount_operations, 1):
                                    print(
                                        f"  {i}. {op['mount_point']} -> {op['remote_path']}"
                                    )

                                print(f"\nResults:")
                                print(
                                    f"  Total mount_info entries: {len(mount_info_list)}"
                                )
                                ...
                                if len(service.active_mounts) == 1:
                                    print(
                                        f"  ✓ Only 1 physical mount (no shadowing possible)"
                                    )
                                else:
                                    print(
                                        f"  ✗ FAIL: {len(service.active_mounts)} mounts (shadowing could occur)"
                                    )
                                    test.teardown()
                                    return False
                                ...
                                if len(unique_mount_ids) == 1:
                                    print(
                                        f"  ✓ Both paths use same mount_id (mount reuse working)"
                                    )
                                else:
                                    print(
                                        f"  ✗ FAIL: Multiple mount IDs (not reusing mounts)"
                                    )
                                    test.teardown()
                                    return False
                                ...
                                for (
                                    mount_id,
                                    mount_info,
                                ) in service.active_mounts.items():
                                    print(f"  ✓ Mount point: {mount_info.mount_point}")
                                    # Should be parent /etc, not child /etc/cron.d
                                    if (
                                        "/etc/cron.d" in mount_info.mount_point
                                        and mount_info.mount_point.endswith(
                                            "/etc/cron.d"
                                        )
                                    ):
                                        print(
                                            f"  ✗ FAIL: Child /etc/cron.d mounted (parent should be mounted)"
                                        )
                                        test.teardown()
                                        return False
                                ...
                                if len(service.active_mounts) == 0:
                                    ...
                                    print(
                                        f"  ✗ FAIL: {len(service.active_mounts)} mounts still active"
                                    )
                                    test.teardown()
                                    return False
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            ...
                            (
                                ...
                                connection_id=1, remote_paths=remote_paths, job_id=888
                            )
                            ...
                            if len(mount_order) == 1:
                                ...
                                print(
                                    f"  ✗ FAIL: {len(mount_order)} mounts created (should be 1)"
                                )
                                return False
                            ...
                            if mount_order[0] == "/var":
                                ...
                                print(
                                    f"  ✗ FAIL: {mount_order[0]} mounted (should be /var)"
                                )
                                return False
