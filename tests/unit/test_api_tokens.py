...
class TestApiTokens:
    ...
    def test_generate_token_returns_full_token_once(self, test_client: TestClient, admin_headers):
    ...
    def test_list_tokens_after_generate_shows_prefix_not_token(self, test_client: TestClient, admin_headers):
        """Token list shows prefix, never the full token"""
        test_client.post("/api/settings/tokens", json={"name": "tok"}, headers=admin_headers)
        response = test_client.get("/api/settings/tokens", headers=admin_headers)
        assert response.status_code == 200
    ...
    def test_generate_token_empty_name_returns_422(self, test_client: TestClient, admin_headers):
    ...
    def test_revoke_token(self, test_client: TestClient, admin_headers):
        ...
        revoke = test_client.delete(f"/api/settings/tokens/{token_id}", headers=admin_headers)
        assert revoke.status_code == 204

        list_resp = test_client.get("/api/settings/tokens", headers=admin_headers)
        assert list_resp.json() == []

    def test_revoke_nonexistent_token_returns_404(self, test_client: TestClient, admin_headers):
        """Revoking a token that does not exist returns 404"""
        response = test_client.delete("/api/settings/tokens/99999", headers=admin_headers)
        assert response.status_code == 404

    def test_revoke_other_users_token_returns_403(self, test_client: TestClient, test_db, admin_headers):
        """Non-admin cannot revoke another user's token"""
        from app.core.security import get_password_hash, create_access_token
        other = User(username="other", password_hash=get_password_hash("pass"), is_active=True, role='viewer')
        test_db.add(other)
        test_db.commit()
        ...
        create = test_client.post("/api/settings/tokens", json={"name": "admin tok"}, headers=admin_headers)
        token_id = create.json()["id"]

        # other user tries to revoke it
        response = test_client.delete(f"/api/settings/tokens/{token_id}", headers=other_headers)
