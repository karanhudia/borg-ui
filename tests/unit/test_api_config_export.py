...
class TestConfigExportTimestamps:
    ...
    def test_single_yaml_export_has_timestamp(
        ...
        assert filename.endswith(".yaml"), (
            f"Filename '{filename}' does not end with .yaml"
        )

    def test_zip_export_has_timestamp(
        self, test_client: TestClient, admin_headers, test_db
    ):
        ...
        response = test_client.post(
            "/api/config/export/borgmatic",
            json={"repository_ids": [repo1.id, repo2.id], "include_schedules": True},
            headers=admin_headers,
        )
    ...
    def test_timestamp_format_is_sortable(
        self, test_client: TestClient, admin_headers, test_db
    ):
        ...
        filename1 = re.search(
            r"filename=([^\s;]+)", response1.headers.get("Content-Disposition")
        ).group(1)
        filename2 = re.search(
            r"filename=([^\s;]+)", response2.headers.get("Content-Disposition")
        ).group(1)
    ...
    def test_fallback_filename_has_timestamp(
        self, test_client: TestClient, admin_headers, test_db
    ):
        ...
        timestamp_pattern = (
            r"^\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}_borgmatic-config\.yaml$"
        )
        assert re.match(timestamp_pattern, filename), (
