#!/usr/bin/env python3
"""Raw Borg 2 CLI progress contract smoke test."""
...
def _require_borg2_binary() -> str | None:
    return shutil.which("borg2")
...
def _detect_app_container() -> str | None:
    ...
    for candidate in ("borg-web-ui-dev", "borg-web-ui"):
        if candidate in result.stdout.splitlines():
            return candidate
    return None
...
from pathlib import Path
base = Path(tempfile.mkdtemp(prefix="borg2-cli-contract-", dir="/tmp"))
repo = base / "repo"
src = base / "source"
repo.mkdir(parents=True, exist_ok=True)
src.mkdir(parents=True, exist_ok=True)
(src / "notes.txt").write_text("borg2 cli contract smoke\n", encoding="utf-8")
...
"""
    ...
    if result.returncode != 0:
        raise SmokeFailure(
            f"Failed to run Borg 2 CLI contract in container {container_name}: {result.stderr or result.stdout}"
        )
...
def main() -> int:
    ...
    if container_name:
        ...
        temp_dir = Path(tempfile.mkdtemp(prefix="borg2-cli-contract-", dir=smoke_root))
        try:
