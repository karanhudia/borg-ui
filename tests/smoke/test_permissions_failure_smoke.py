...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run permissions and failure smoke tests"
    )
    ...
    try:
        ...
        if len(archives) != 1:
            raise SmokeFailure(
                f"Expected one archive before permission checks, got {archives}"
            )
        ...
        if len(viewer_archives) != 1:
            raise SmokeFailure(
                f"Viewer should see repository archives, got {viewer_archives}"
            )

        restore_items = client.restore_contents(
            repo_id, archive_name, token=viewer_token
        )
        ...
        if download_bytes != b"viewer-visible\n":
            raise SmokeFailure(
                f"Viewer archive download returned wrong content: {download_bytes!r}"
            )
        ...
        for method, path, payload, params in [
            ...
            if response.status_code != 403:
                raise SmokeFailure(
                    f"Viewer mutation {method} {path} should return 403, got {response.status_code}: {response.text}"
                )

        missing_repo_job = client.start_backup(str(client.temp_dir / "missing-repo"))
        failed_job = client.wait_for_job(
            "/api/backup/status", missing_repo_job, expected={"failed"}, timeout=45
        )
        if failed_job["status"] != "failed":
            raise SmokeFailure(
                f"Missing repository backup should fail, got {failed_job}"
            )

        missing_archive_response = client.request(
            "GET",
            f"/api/archives/does-not-exist/info",
