...
def main() -> int:
    ...
    try:
        ...
        job_data = client.wait_for_job(
            "/api/backup/status", backup_job_id, expected={"failed"}, timeout=45
        )
        if job_data["status"] != "failed":
            raise SmokeFailure(f"Expected failed backup job, got {job_data}")

        logs_response = client.request_ok(
            "GET", f"/api/backup/logs/{backup_job_id}/download"
        )
        disposition = logs_response.headers.get("content-disposition", "")
        if "backup_job_" not in disposition or not logs_response.content:
            raise SmokeFailure(
                f"Unexpected backup logs download response: headers={logs_response.headers}"
            )

        body = logs_response.text.lower()
        if "repository" not in body and "not found" not in body:
            raise SmokeFailure(
                f"Expected missing-repository error in logs, got: {logs_response.text[:400]}"
            )
