...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run restore-to-SSH-destination smoke test")
    ...
    try:
        ...
        repo_id, repo_path, _backup_job_id, _backup = client.create_repository_and_backup(
        ...
        if len(archives) != 1:
            raise SmokeFailure(f"Expected one archive before restore-to-SSH, got {archives}")
        archive_name = archives[0]["name"]

        destination_root = Path(args.remote_root) / f"restore-dest-{client.temp_dir.name}"
        ...
        client.wait_for_job("/api/restore/status", restore_job_id, expected={"completed"}, timeout=180)

        restored_files = sorted(path for path in destination_root.rglob("remote-dest.txt") if path.is_file())
        if not restored_files:
            all_files = sorted(path.relative_to(destination_root).as_posix() for path in destination_root.rglob("*") if path.is_file())
            raise SmokeFailure(f"Restore to SSH destination did not produce expected file: {all_files}")
        restored_file = restored_files[0]
        if restored_file.read_text(encoding="utf-8") != "restore to ssh destination\n":
            raise SmokeFailure(f"Unexpected restore-to-SSH file contents: {restored_file.read_text(encoding='utf-8')!r}")
