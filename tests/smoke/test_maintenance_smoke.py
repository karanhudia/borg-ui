...
def main() -> int:
    ...
    try:
        ...
        for index in range(3):
            (source_root / f"archive-{index}.txt").write_text(
                f"version {index}\n", encoding="utf-8"
            )
        ...
        if len(archives_before) < 3:
            raise SmokeFailure(
                f"Expected at least 3 archives before prune, got {archives_before}"
            )
        ...
        if len(archives_after) != 1:
            raise SmokeFailure(
                f"Expected prune to leave one archive, got {archives_after}"
            )
        ...
        break_lock_response = client.request_ok(
            "POST", f"/api/repositories/{repo_id}/break-lock"
        )
        if break_lock_response.status_code != 200:
            raise SmokeFailure(
                f"Unexpected break-lock response: {break_lock_response.text}"
            )
