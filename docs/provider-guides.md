...
Hetzner offer three Borg binaries on the box. Match Remote Borg Path to the
Borg version the repository was created with:

| Repository created with | Remote Borg Path |
| ----------------------- | ---------------- |
| Borg 1.1                | `borg-1.1`       |
| Borg 1.2                | blank, this is Hetzner's default |
| Borg 1.4                | `borg-1.4`       |

Each sub-account needs its own `authorized_keys` file in its own directory.
...
If you would rather not hand Borg UI the Storage Box password, upload the key
yourself. Hetzner's Console cannot add an SSH key to a Storage Box that already
exists, so it has to go onto the host server. With OpenSSH 8.5 or newer:

```bash
ssh-copy-id -p 23 -s u123456@u123456.your-storagebox.de
```

The `-s` flag is OpenSSH 8.5 and newer only; older clients reject it. For those,
copy the key across by hand:

```bash
ssh -p 23 u123456@u123456.your-storagebox.de mkdir .ssh
scp -P 23 ~/.ssh/id_rsa.pub u123456@u123456.your-storagebox.de:.ssh/authorized_keys
```
...
| `Permission denied (publickey)`                              | SSH support is off on the Storage Box, or the key never landed in `authorized_keys`. | Enable SSH Support in the Hetzner Console under Change settings, then run Deploy key again. The Console cannot add a key to an existing Storage Box, so the key has to go on the host server. See the manual key upload below. |
