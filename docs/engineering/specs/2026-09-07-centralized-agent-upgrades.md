...
| 4. Fleet upgrade | done | `docs/engineering/plans/2026-09-10-agent-upgrades-phase-4.md` | `feat/agent-upgrades-phase-4` (PR #997) | Queued endpoints do not age towards the reaper timeout; waves release in id order; merged to main 2026-09-10 as `a640a11e` with the review fixes included, approved, CI green |
| 5. Per-endpoint Borg version | done | `docs/engineering/plans/2026-09-10-agent-upgrades-phase-5.md` | `feat/agent-upgrades-phase-5` (PR #1007) | Plan approved at G1 on 2026-09-10 with all seven Open questions taking their defaults. Drafted and implemented on Opus 5; this spec's phase table names no model per phase, so no G0 check applied. Six tasks. Design: `/agent/install.sh` and its `.sha256` finally read the `agent_id` the phase 2 helper has always sent, so the served script carries that endpoint's pins (`InstallerPins`, whitelisted at render time because the block runs as root); one new `apply_pinned_borg_version` in the script lets a served pin beat both the helper's `--borg-version` from `upgrade.conf` and reinstall mode's skip-by-default, and `write_upgrade_conf` then records it so it sticks; a pinned Borg 2 on a `distro` endpoint falls back to the server's binaries; `resolve_agent_upgrade` waits for the pinned major to appear in the reported `borg_versions` (a Borg only change never moves `agent_version`) and its two call sites move below the `borg_versions` assignment, which they preceded; `AgentBorgVersionChip` on the card; the pin selector already shipped in phase 3 and only its hint copy changes. `desired_agent_version` write-through is included, per spec section 6. Seven Open questions approved at G1 with their defaults. Implemented 2026-09-10: 3909 backend unit tests and 2630 frontend tests pass, ruff and oxlint clean, tsc and build clean, no em dashes in added lines. Two defects found while implementing and fixed here: `/agent/install.sh` and its `.sha256` had been ignoring the `agent_id` the phase 2 helper always sent, and `resolve_agent_upgrade` ran before `borg_versions` was stored on both the heartbeat and hello paths. The phase gate's live Borg 1 to Borg 2 move is not yet run by hand; Storybook snapshots not run (Node 20.19+ via fnm). Local CodeRabbit review against origin/main returned zero findings across all 20 files; rebased onto 39ee2e3c and opened as PR #1007. The GitHub review then raised three comments the local run had not: an agent version pin ending in a newline passed the whitelist, because `$` in a `re.match` also matches before a trailing newline (fixed with `fullmatch` plus regression tests), and the card's chip row could overflow a 375px card now that a fourth chip can appear (fixed with wrap and shrink). The sync pin lookup on the event loop was declined: it is the convention in `app/api` (257 async handlers query on the loop against 1 sync handler), the query is one unique-indexed lookup, and the slow render is already off the loop. Merged to main 2026-09-10 via PR #1007, approved with CI green. The phase gate's live Borg 1 to Borg 2 move is left to the owner to verify on a real endpoint; every other check is automated. |
