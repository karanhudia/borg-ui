...
| `off` | no | no | no | Slow or rarely reachable remotes. Nothing derived is refreshed; the archive list, status strip, and dashboard age go stale for this repository and say so. |
...
- Managed-agent repositories accept `full` and `archives` only; `off` is
  rejected with a 422 until the agent kinds migrate in phase 5, since the
  agent's own stats writer (noted in the phase 1 progress row) would keep
  running regardless.
...
- The repository status strip (10.2) omits the history category for
  `archives` and shows one line for `off`: "Background work is off for this
  repository", linking to the setting.
...
| 10 | Per-repository index mode (6.8): `index_mode` column and migration; mode-aware `chain_for` and reconcile; cancel queued index work on change, catch-up run on return to `full`; validation for agent repositories; repository settings control for the mode and the 6.7 exclude list; hub row, hub summary counts, status strip, and archive route states; `docs/api.md` and user docs | Sonnet 5 for the column, chain filter, routes, and settings control; Opus 5 for the hub and status strip states | Opus 5 | A small change that touches every surface of the feature; the reviewer checks that no surface reads an opted-out repository as broken. Depends on phase 4 only and may run before or after phases 5 to 9. |
