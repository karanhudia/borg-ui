...
The Background work tab and the Archives page subscribe through the existing
SSE hook.
...
### 10.2 Repository status strip (withdrawn)

Shipped in phase 3 as `OperationStatusStrip` inside `RepositoryCard`, fed by
`GET /repositories/{id}/status-strip`, and removed again before phase 5. The
section number is kept so the phase 3 log and the plans that reference it
still resolve.

Why it was withdrawn:

- Two of its six cells restated key stats the card already showed, from a
  different source, so a repository backed up outside Borg UI read "today" in
  the key stat and "overdue" in the strip on the same card (#935).
- It was a per-card query with a 30 second refetch, roughly 15 to 18 database
  queries per repository per refresh, on a page that renders every repository.
- The remaining signal, "which repositories are behind", is the operations
  hub's job, and the hub answers it for the whole fleet in one query.

Nothing replaces it on the card. If a per-repository health signal is wanted
later, it belongs in the repositories list payload as one badge, not as its
own polled route. The `overdue` rule and `OVERDUE_THRESHOLD_DAYS` went with
it, as did `legacy_status.py`, which phase 9 would otherwise have deleted.
...
| Size and duration outlier flags | no | yes |
...
- The heatmap route always returns counts, sizes, and `missed_run`; it
  includes outlier flags only when the plan includes the feature.
...
| `frontend/src/components/RepositoryCard.tsx` | Repository card | Gains the Operations action (phase 3) |
...
| The repository status strip (10.2) is withdrawn after phase 3 rather than carried into phases 5 to 9 | Keep it and give it one status model per repository, per #935 | Two of its six cells restated key stats the card already showed, from a second source that disagreed with the first; it cost a polled query per card on a page that renders every repository; and the operations hub now answers "which repositories are behind" for the whole fleet in one query |
