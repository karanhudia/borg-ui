...
5. **Add Source Paths**
   ...
   ** Important:** Use `/local/` prefix for paths inside the container!
...
1. ** Restrict Volume Mounts (Critical)** - Never use `/:/local:rw` in production. Mount only the specific directories you need:
   ```yaml
   volumes:
