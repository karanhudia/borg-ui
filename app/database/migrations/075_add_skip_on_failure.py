...
def upgrade(db):
    rs_cols = [
        row[1]
        for row in db.execute(text("PRAGMA table_info(repository_scripts)")).fetchall()
    ]
    if "skip_on_failure" not in rs_cols:
        ...
        print(
            "✓ skip_on_failure column already exists in repository_scripts — skipping"
        )

    repo_cols = [
        row[1] for row in db.execute(text("PRAGMA table_info(repositories)")).fetchall()
    ]
