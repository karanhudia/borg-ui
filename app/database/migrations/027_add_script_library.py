...
def upgrade(db):
    ...
    try:
        ...
        cursor = db.execute(
            text(
                "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('scripts', 'repository_scripts', 'script_executions')"
            )
        )
        ...
        if "repository_scripts" not in existing_tables:
            ...
            db.execute(
                text(
                    "CREATE INDEX ix_repository_scripts_repository_id ON repository_scripts(repository_id)"
                )
            )
            db.execute(
                text(
                    "CREATE INDEX ix_repository_scripts_script_id ON repository_scripts(script_id)"
                )
            )
            db.execute(
                text(
                    "CREATE INDEX ix_repository_scripts_hook_type ON repository_scripts(hook_type)"
                )
            )
        ...
        if "script_executions" not in existing_tables:
            ...
            db.execute(
                text(
                    "CREATE INDEX ix_script_executions_script_id ON script_executions(script_id)"
                )
            )
            db.execute(
                text(
                    "CREATE INDEX ix_script_executions_repository_id ON script_executions(repository_id)"
                )
            )
            db.execute(
                text(
                    "CREATE INDEX ix_script_executions_backup_job_id ON script_executions(backup_job_id)"
                )
            )
            db.execute(
                text(
                    "CREATE INDEX ix_script_executions_status ON script_executions(status)"
                )
            )
