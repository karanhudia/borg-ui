...
def upgrade(db):
    ...
    db.commit()
    logger.info(
        "Migration 087_add_repository_inline_script_parameters completed successfully"
    )


def downgrade(db):
    logger.warning(
        "Downgrade not supported for SQLite - repository inline script parameter columns will remain"
    )
