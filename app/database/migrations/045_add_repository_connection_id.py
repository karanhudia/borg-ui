...
def upgrade(connection):
    ...
    result = connection.execute(text("PRAGMA table_info(repositories)")).fetchall()
    ...
    if "connection_id" not in column_names:
        ...
        connection.execute(
            text("ALTER TABLE repositories ADD COLUMN connection_id INTEGER")
        )
...
def downgrade(connection):
    ...
    connection.execute(text("DROP INDEX IF EXISTS idx_repositories_connection_id"))
