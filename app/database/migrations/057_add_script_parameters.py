...
def upgrade(db):
    ...
    try:
        ...
        if "parameter_values" not in repository_scripts_columns:
            ...
            print(
                "⊘ parameter_values column already exists in repository_scripts table"
            )
...
def downgrade(db):
    ...
    try:
        ...
        if (major, minor) >= (3, 35):
            ...
            db.execute(
                text("ALTER TABLE repository_scripts DROP COLUMN parameter_values")
            )
            ...
            print(
                "  To fully remove: upgrade to SQLite 3.35+ or manually recreate tables"
            )
