...
def upgrade(db):
    ...
    try:
        ...
        try:
            db.execute(
                text(
                    "ALTER TABLE repository_scripts ADD COLUMN continue_on_error BOOLEAN DEFAULT 1"
                )
            )
            ...
            if "duplicate column" in error_str or "already exists" in error_str:
                ...
                try:
                    db.execute(
                        text("SELECT continue_on_error FROM repository_scripts LIMIT 1")
                    )
        ...
        db.execute(
            text(
                "UPDATE repository_scripts SET continue_on_error = 1 WHERE continue_on_error IS NULL"
            )
        )
        print(
            "✓ Updated existing repository_scripts entries to default continue_on_error=True"
        )
