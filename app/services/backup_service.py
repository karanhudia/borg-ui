...
class BackupService:
    ...
    async def _calculate_source_size(
        ...
        try:
            ...
            logger.info(
                "Starting source size calculation",
                paths=source_paths,
            ...
            for path in source_paths:
                try:
                    ...
                    if path.startswith("ssh://"):
                        ...
                        if match:
                            ...
                            stdout, stderr = await asyncio.wait_for(
                                process.communicate(), timeout=source_size_timeout
                            )
                            ...
                            if process.returncode == 0:
                                ...
                                if output and output.isdigit():
                                    ...
                                    logger.info(
                                        "Calculated remote directory size",
                                        path=path,
                                        size=path_size,
                                        size_formatted=self._format_bytes(path_size),
                                    )
                                else:
                                    logger.warning(
                                        "Failed to parse remote directory size",
                                        path=path,
                                        output=output,
                                    )
                            else:
                                logger.warning(
                                    "Failed to calculate remote directory size",
                                    path=path,
                    ...
                    logger.warning(
                        "Timeout while calculating directory size",
                        path=path,
            ...
            if total_size > 0:
                logger.info(
                    "Total source size calculated successfully",
                ...
                logger.warning(
                    "Source size calculation returned 0 - all paths failed or were empty",
                    paths=source_paths,
            ...
            logger.error(
                "Failed to calculate total source size",
                error=str(e),
                error_type=type(e).__name__,
                paths=source_paths,
            )
    ...
    async def execute_backup(
        self, job_id: int, repository: str, db: Session = None, archive_name: str = None
    ):
        ...
        try:
            ...
            if job.source_ssh_connection_id:
                ...
                _sc = (
                    db.query(_SC).filter(_SC.id == job.source_ssh_connection_id).first()
                )
                if _sc:
                    _src_conn_info = {
                        "host": _sc.host,
            ...
            ssh_opts = [
                ...
                "-o",
                "RequestTTY=no",  # Disable TTY allocation to prevent shell initialization output
                "-o",
                "PermitLocalCommand=no",  # Prevent local command execution
            ]
            ...
            try:
                ...
                if repo_record:
                    ...
                    if repo_record.passphrase:
                        ...
                        logger.info(
                            "Using passphrase from repository record",
            ...
            if repo_record and repo_record.connection_id:
                ...
                connection = (
                    db.query(SSHConnection)
                    .filter(SSHConnection.id == repo_record.connection_id)
                    .first()
                )
                ...
                repo_record
                and repo_record.repository_type == "ssh"
                and repo_record.ssh_key_id
            ):
                # FALLBACK: Legacy repos without connection_id (backward compatibility)
                ssh_key = (
                    db.query(SSHKey).filter(SSHKey.id == repo_record.ssh_key_id).first()
                )
            ...
            async def check_cancellation():
                ...
                while not cancelled and process.returncode is None:
                    await asyncio.sleep(
                        3
                    )  # Check every 3 seconds (reduced from 1s for performance)
                    db.refresh(job)
