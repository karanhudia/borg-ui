...
async def debug_network(
    host: Optional[str] = Query(None, description="Test connectivity to specific host"),
    current_user: User = Depends(get_current_user)
):
    """Debug endpoint to test network connectivity (admin only)"""
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    try:
        ...
        try:
            ...
            result["container_info"]["interfaces"] = ifconfig_output.stdout
        except Exception as e:
            result["container_info"]["interfaces"] = f"Error: {str(e)}"
        ...
        try:
            ...
            result["container_info"]["routes"] = route_output.stdout
        except Exception as e:
            # ip command might not be available, try netstat
            try:
                route_output = subprocess.run(
                ...
                result["container_info"]["routes"] = f"Error: {str(e2)}"

        # Test specific host if provided
        if host:
            # Ping test
            try:
                ...
                result["connectivity_tests"][f"ping_{host}"] = {
            ...
            try:
                traceroute_result = subprocess.run(
                    ["traceroute", "-m", "5", "-w", "2", host],
                    capture_output=True, text=True, timeout=15
                )
                result["connectivity_tests"][f"traceroute_{host}"] = {
        ...
        test_hosts = ["8.8.8.8", "1.1.1.1", "host.docker.internal"]
        for test_host in test_hosts:
            try:
                ...
                result["connectivity_tests"][f"ping_{test_host}"] = {
                    "success": ping_result.returncode == 0
                }
            except Exception:
                result["connectivity_tests"][f"ping_{test_host}"] = {
        ...
        return {
            "success": True,
            "network_info": result,
            "help": "See NETWORK_SETUP.md for configuration guidance"
        }
    except Exception as e:
        logger.error("Failed to get network debug info", error=str(e))
        raise HTTPException(status_code=500, detail=f"Failed to get network info: {str(e)}")

@router.get("/debug/database")
async def debug_database(
    table: Optional[str] = Query(None, description="Specific table to query"),
    query: Optional[str] = Query(None, description="Custom SQL query"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Debug endpoint to view database contents (admin only)"""
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    try:
        ...
        if query:
            ...
            with engine.connect() as conn:
                ...
                result["query_result"] = {"count": len(rows), "columns": list(columns), "data": rows}
        elif table:
            # Query specific table
            with engine.connect() as conn:
                query_result = conn.execute(text(f"SELECT * FROM {table}"))
                columns = query_result.keys()
                rows = [dict(zip(columns, row)) for row in query_result.fetchall()]
                result[table] = {"count": len(rows), "data": rows}
        else:
            # Query all tables
            tables = ["users", "ssh_keys", "ssh_connections", "repositories", "backup_jobs"]
            for tbl in tables:
                try:
                    with engine.connect() as conn:
                        query_result = conn.execute(text(f"SELECT * FROM {tbl}"))
                        columns = query_result.keys()
                        rows = [dict(zip(columns, row)) for row in query_result.fetchall()]
                        # Hide sensitive data
                        if tbl == "users":
                        ...
                        if tbl == "ssh_keys":
                            for row in rows:
                                if "private_key" in row:
                                    row["private_key"] = "***HIDDEN***"
                        result[tbl] = {"count": len(rows), "data": rows}
                except Exception as e:
                    result[tbl] = {"error": str(e)}

        return {
            "success": True,
            "database": app_settings.database_url.split("///")[-1] if "///" in app_settings.database_url else "unknown",
            "tables": result
        }
    except Exception as e:
