...
async def create_scheduled_job(
    ...
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403,
            detail={"key": "backend.errors.schedule.adminAccessRequired"},
        )
    ...
    try:
        ...
        try:
            cron = croniter.croniter(
                job_data.cron_expression, datetime.now(timezone.utc)
            )
        ...
        if job_data.repository:
            repo = (
                db.query(Repository)
                .filter(Repository.path == job_data.repository)
                .first()
            )
...
async def duplicate_scheduled_job(
    ...
    try:
        ...
        try:
            cron = croniter.croniter(
                original_job.cron_expression, datetime.now(timezone.utc)
            )
            next_run = cron.get_next(datetime)
        except Exception as e:
            raise HTTPException(
    ...
    for repo in repositories:
        try:
            ...
            await backup_service.execute_backup(
                backup_job.id, repo.path, db, archive_name=archive_name
            )
    ...
    try:
        ...
        if scheduled_job.run_compact_after and (
            ...
            try:
                ...
                await compact_service.execute_compact(compact_job.id, repo.id, db)
                ...
                try:
                    ...
                    if repo_links:
                        ...
                        if job.repository_id:
                            repo = (
                                db.query(Repository)
                                .filter_by(id=job.repository_id)
                                .first()
                            )
                        else:
                            repo = (
                    ...
                    logger.error(
                        "Failed to run scheduled job", job_id=job.id, error=str(e)
                    )
