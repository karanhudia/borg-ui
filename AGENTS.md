...
- Storybook visual snapshots are generated by Argos CI for pull requests. Do not commit generated PNGs under `frontend/storybook-snapshots/` or `frontend/argos-screenshots/`.
- For local visual proof, run `cd frontend && npm run snapshots`; it writes ignored screenshots under `frontend/argos-screenshots/`.
- UI components should stay small and composed from smaller components. When a UI component is added or changed, add or update the component story that covers it.
